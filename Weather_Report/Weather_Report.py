...
def read_weather_data():    
    def read_province_region_data():
        region_of = {}       
        f = open('provinces.txt', encoding='utf-8-sig')    
        for line in f:      
...
def approx_match(stations, name):
    ...
    for i in stations:
        formatStation = "" # maehongson
        for j in i: # i = "MAE HONG SON"
            if j != " ":
                formatStation += j.lower()
...
def top_k_min_temp_stations(stations, K):
    stationTemp = {}
    for i in stations:
        stationTemp[i] = stations[i]['temp']

    allTemp = []
    for i in stationTemp:
        if stationTemp[i] not in allTemp:
            allTemp.append(stationTemp[i])
    allTemp.sort()                        
    ...
    while K > 0:
        sameTempStation = []
        for i in stationTemp:
            if stationTemp[i] == allTemp[a]:
                sameTempStation.append(i)
        sameTempStation.sort()
        for i in sameTempStation:
            minTemp.append(i)
            K-=1
            if K == 0:
                break
...
def peak_stations(stations):
    ...
    for i in stations:
        if stations[i]['lat'] > maxLat[1]:
            maxLat[1] = stations[i]['lat']
            maxLat[0] = i
        if stations[i]['long'] > maxLong[1]:
            maxLong[1] = stations[i]['long']
            maxLong[0] = i
        if stations[i]['lat'] < minLat[1]:
            minLat[1] = stations[i]['lat']
            minLat[0] = i
        if stations[i]['long'] < minLong[1]:
            minLong[1] = stations[i]['long']
            minLong[0] = i

    return [minLat[0],maxLat[0],minLong[0],maxLong[0]]

def k_nearby_stations(stations, main_station, K):
    mainLat = stations[main_station]['lat']
    mainLong = stations[main_station]['long']

    stationDis = {}
    for i in stations:
        if i != main_station:
            stationDis[i] = distance(mainLat,mainLong,stations[i]['lat'],stations[i]['long'])
    
    allDis = []
    for i in stationDis:
        if stationDis[i] not in allDis:
            allDis.append(stationDis[i])
    allDis.sort()
    # print(stationDis)
    a = 0
    minDis = []
    while K > 0:
        sameDistance = []
        for i in stationDis:
            if stationDis[i] == allDis[a]:
                sameDistance.append(i)
        sameDistance.sort()
        for i in sameDistance:
            minDis.append(i)
            K -= 1
            if K == 0:
                break
...
def average_temp(stations, names):
    ...
    totalTemp = 0
    for i in names:
        totalTemp += stationTemp[i]

    return totalTemp/len(names)

# print(k_nearby_stations(stations,'PRACHIN BURI',10))
# print(peak_stations(stations))
# print(top_k_min_temp_stations(stations,5))
# --------------------------------------------------
main()
